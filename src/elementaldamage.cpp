...
void __fastcall CUIToolTip__SetToolTip_Equip_Basic_hook(CUIToolTip* pThis, void* _EDX, GW_ItemSlotBase* pe) {
    ...
    PrintElementDamage(pThis, "¶¾ÊôÐÔÉËº¦ :", pEquipItem->nirPoison);   // incRMAS
    PrintElementDamage(pThis, "±ùÊôÐÔÉËº¦ :", pEquipItem->nirIce);         // incRMAI
    PrintElementDamage(pThis, "»ðÊôÐÔÉËº¦ :", pEquipItem->nirFire);       // incRMAF
    PrintElementDamage(pThis, "À×ÊôÐÔÉËº¦ :", pEquipItem->nirLight); // incRMAL
    PrintElementDamage(pThis, "Ê¥ÊôÐÔÉËº¦ :", pEquipItem->nirHoly);       // incRMAH
