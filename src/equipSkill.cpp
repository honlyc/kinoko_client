...
// Store the calculated tooltip height increasement after CalcTooltipHeightIncrease
...
int __stdcall CalcTooltipHeightIncrease(CUIToolTip* cuitooltip, EquipSkill* equipskill) {
    ...
    // Calculate the height of Expire info
    ...
    // Calculate the height of Skill Desc
    ...
    // Calculate the height of Skill Level Desc
    if (!(nDisplayType & 4)) {
...
// Draw Skill icon background with the color corresponding to skill quality
...
// Hook to draw equip skill with Item Desc available
...
// Hook to draw equip skill without Item Desc
